...
export async function updateChallenge(id, data) {
  return updateDoc(doc(db, 'challenges', id), {
    ...data,
...
export async function getLeaderboard() {
  try {
    const snap = await getDocs(
      query(collection(db, 'scores'), orderBy('points', 'desc'))
    )
...
export async function getCTFSettings() {
  try {
    ...
    return snap.exists()
      ? snap.data()
      : { name: 'DragonByte CTF', active: true }
...
// ─── ADMIN: GET ALL SUBMISSIONS ✅ (FIXED ERROR)
export async function getAllSubmissions() {
  try {
    const snap = await getDocs(
      query(collection(db, 'submissions'), orderBy('solvedAt', 'desc'))
    )
    return snap.docs.map(d => ({ id: d.id, ...d.data() }))
  } catch (e) {
    console.error(e)
