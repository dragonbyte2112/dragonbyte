        ...
        <div style={{ padding:'0 1.5rem 1.5rem', borderTop:'1px solid #0f3020' }}>
          ...
          {challenge.attachmentUrl && (
            <div style={{ marginBottom:'1rem' }}>
              <a href={challenge.attachmentUrl} target="_blank" rel="noreferrer"
                style={{ fontFamily:'"Share Tech Mono",monospace', fontSize:'0.72rem', color:'#00d4ff', letterSpacing:'1px', textDecoration:'none', border:'1px solid #0099cc', padding:'6px 14px', borderRadius:4, display:'inline-block' }}>
                📎 DOWNLOAD ATTACHMENT
              </a>
            </div>
          )}
...
export default function ChallengesContent() {
  ...
  useEffect(() => {
    const unsub = listenChallenges(data => {
      setChallenges(data.filter(c => c.published))
      setLoading(false)
    })
    return () => unsub()
  }, [])
      ...
      <div style={{ padding:'3rem 2rem 1rem', maxWidth:1100, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', alignItems:'center', marginBottom:'1rem' }}>
          ...
          <select onChange={e => setSortBy(e.target.value)} value={sortBy}
            style={{ background:'#071a0e', border:'1px solid #0f3020', borderRadius:4, padding:'8px 12px', color:'#b0ffcc', fontFamily:'"Share Tech Mono",monospace', fontSize:'0.7rem', outline:'none', cursor:'pointer' }}>
